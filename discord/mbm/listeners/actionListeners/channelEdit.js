...
const listener = {
    ...
    async listener (log, guild) {
        if (log.targetType !== "Channel" || log.actionType !== "Update") return;

        const changes = log.changes.filter(x => validChanges.find(y => y.name === x.key));
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`A new channel <#${log.targetId}> was edited${executor ? ` by <@${executor.id}>` : ""} at <t:${Math.floor(currentDate / 1000)}:f>`)
            .setColor(0x772ce8)
        ...
        validChanges.forEach(validChange => {
            ...
            embed.addFields([
                {
                    name: `Old ${validChange.title}`,
                    value: validChange.transform(String(change.old)),
                    inline: true,
                },
                {
                    name: `New ${validChange.title}`,
                    value: validChange.transform(String(change.new)),
                    inline: true,
                },
                {
                    name: "\u200B",
